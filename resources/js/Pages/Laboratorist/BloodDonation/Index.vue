    ...
    <AppLayout>
        ...
        <div class="mb-4 flex justify-between">
            ...
            <div class="flex items-center space-x-2 sm:space-x-3 ml-auto">
                <BaseButton v-if="filter.keyword && filter.keyword.length" as="link" :href="route('laboratorist.bloodDonation.index')" class="text-white bg-red-600 hover:bg-red-700 px-3 py-2">
       ...
       <template v-else-if="!loading && blood_donations && blood_donations.data.length">
            <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
                <span v-for="blood_donation in blood_donations.data" :key="blood_donation.id" class="block p-6 bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                    <div class="flex flex-wrap justify-between items-start">
                        <h2 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ blood_donation.blood_donor?.name }}</h2>
                    ...
                    <h2 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ blood_donation.blood_donor?.blood_group ?? '' }}</h2>
...
<script>
    ...
    export default {
        ...
        methods: {
            deleteData(id) {
                this.$swal({
                    ...
                    if (result.isConfirmed) {
                        this.$inertia.delete(route("laboratorist.bloodDonation.destroy", id));
                    }
                });
            },
            ...
            filterData(){
                this.loading = true
                this.filterForm.get(route('laboratorist.bloodDonation.index'), {
                    onSuccess: () => {
                        this.loading = false
                    },
                    onError: () => {
            ...
            toggleFilter() {
                ...
                localStorage.setItem("laboratoristBloodDonation", this.showFilter);
            },
        },
        created() {
            this.showFilter = localStorage.getItem("laboratoristBloodDonation") == "true" ? true: false;
        },
    };
</script>
