<template>
    <AppLayout title="Blood Donor">
        ...
        <!-- Header Part  -->
        <Breadcrumb>
            <BreadcrumbLink title="Blood Donor"/>
        </Breadcrumb>
        ...
        <div class="mb-4 flex justify-between">
            ...
            <div class="flex items-center space-x-2 sm:space-x-3 ml-auto">
                ...
                <!-- Clear Filter -->
                <ClearFilter v-if="filter.keyword || filter.gender || filter.blood_group" :href="route('laboratorist.bloodDonor.index')"/>
        ...
        <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-100" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
            <div v-if="showFilter" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-5 mb-4 bg-white rounded-lg shadow-xs  items-center p-4">
                ...
                <div>
                    <label for="laboratorist_blood_group" class="block text-sm font-medium text-gray-700">{{ __('Blood Group') }}</label>
                    <div class="mt-1">
                        <Multiselect id="laboratorist_blood_group" :close-on-select="true" :can-clear="true"
                            :searchable="true" v-model="filterForm.blood_group" :create-option="false"
                            placeholder="Blood Group" :options="bloodGroups"  />
                    </div>
                </div>
                <div>
                    <label for="laboratorist_blood_donor_gender" class="block text-sm font-medium text-gray-700">{{ __('Gender') }}</label>
                    <div class="mt-1">
                        <Multiselect id="laboratorist_blood_donor_gender" :close-on-select="true" :can-clear="true"
        ...
        <!-- Card View  -->
       <template v-else-if="!loading && blood_donors && blood_donors.data.length && viewType == 'card'">
            <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
                <span v-for="blood_donor in blood_donors.data" :key="blood_donor.id" class="block p-6 bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100   ">
                    <div class="flex flex-wrap justify-between items-start">
                        <h2 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 ">{{ blood_donor.name }}</h2>
        ...
        <!-- Table View  -->
        <BaseTable v-else-if="!loading && blood_donors && blood_donors.data.length && viewType == 'table'" :items="blood_donors">
            <template v-slot:head>
                <tr class="divide-x divide-gray-200">
                    ...
                    <th class="px-4 py-3.5 text-left text-sm font-semibold text-gray-900 break-words">Age</th>
                    <th width="80px" class="py-3.5 pl-4 pr-4 text-left text-sm font-semibold text-gray-900 sm:pr-6">Action</th>
                </tr>
            </template>
            <template v-slot:body>
                <tr v-for="blood_donor in blood_donors.data" :key="blood_donor.id" class="divide-x divide-gray-200">
...
<script>
    ...
    export default {
        ...
        methods: {
            changeViewType(type){
                this.viewType = type
                localStorage.setItem("laboratoristBloodDonorViewType", this.viewType);
            },
            deleteData(id) {
                this.$swal({
                    title: "Are you sure?",
                    ...
                    if (result.isConfirmed) {
                        this.$inertia.delete(route("laboratorist.bloodDonor.destroy", id));
                    }
                });
            },
            ...
            filterData(){
                this.loading = true
                this.filterForm.get(route('laboratorist.bloodDonor.index'), {
                    onSuccess: () => {
                        this.loading = false
                    },
                    onError: () => {
            ...
            toggleFilter() {
                ...
                localStorage.setItem("laboratoristBloodDonor", this.showFilter);
            },
        },
        created() {
            this.showFilter = localStorage.getItem("laboratoristBloodDonor") == "true" ? true: false;
            this.viewType = localStorage.getItem("laboratoristBloodDonorViewType") == "card" ? 'card': 'table';
        },
    };
</script>
