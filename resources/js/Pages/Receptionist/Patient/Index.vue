    ...
    <AppLayout title="Patient">
        ...
        <div class="mb-4 flex justify-between">
            ...
            <div class="flex items-center space-x-2 sm:space-x-3 ml-auto">
                ...
                <BaseButton v-if="filter.keyword && filter.keyword.length" as="link" :href="route('receptionist.pharmacist.index')" class="text-white bg-red-600 hover:bg-red-700 px-3 py-2">
                        ...
                        <MenuItems class="absolute right-0 z-10 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                            <div class="py-1 text-sm">
                                <MenuItem v-slot="{ active }">
                                    <a :href="route('receptionist.patient.export', 'pdf')" target="_blank" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
                                        <font-awesome-icon icon="fa-solid fa-file-pdf" class="h-6 w-6 mr-2 text-red-500 group-hover:text-red-500"/>
                                        As PDF
                                    </a>
                                </MenuItem>
                                <MenuItem v-slot="{ active }">
                                    <a :href="route('receptionist.patient.export', 'csv')" target="_blank" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
                                        <font-awesome-icon icon="fa-solid fa-file-csv" class="h-6 w-6 mr-2 text-blue-500 group-hover:text-blue-500"/>
                                        As CSV
                                    </a>
                                </MenuItem>
                                <MenuItem v-slot="{ active }">
                                    <a :href="route('receptionist.patient.export', 'xlsx')" target="_blank" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
       ...
       <template v-else-if="!loading && patients && patients.data.length && viewType == 'card'">
            <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
                <span v-for="patient in patients.data" :key="patient.id" class="block p-6 bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100">
                    <div class="flex flex-wrap justify-between items-start">
                        <div class="relative mb-5">
                            <span>
                                <img class="w-16 h-16 rounded object-cover" alt="image" :src="patient.avatar">
                            </span>
                        </div>
                        <Menu as="div" class="relative inline-block text-left">
                            <div>
        ...
        <!-- Table View  -->
        <BaseTable v-else-if="!loading && patients && patients.data.length && viewType == 'table'" :items="patients">
...
<script>
    ...
    export default {
        ...
        methods: {
            changeViewType(type){
                this.viewType = type
                localStorage.setItem("receptionistPatientView", this.viewType);
            },
            deleteData(id) {
                this.$swal({
                    title: "Are you sure?",
                    ...
                    if (result.isConfirmed) {
                        this.$inertia.delete(route("receptionist.patient.destroy", id));
                    }
                });
            },
            editData(data){
            ...
            filterData(){
                this.loading = true
                this.filterForm.get(route('receptionist.patient.index'), {
                    onSuccess: () => {
                        this.loading = false
                    },
                    onError: () => {
            ...
            toggleFilter() {
                ...
                localStorage.setItem("receptionistPatient", this.showFilter);
            },
        },
        created() {
            this.showFilter = localStorage.getItem("receptionistPatient") == "true" ? true: false;
            this.viewType = localStorage.getItem("receptionistPatientView") == "card" ? 'card': 'table';
        },
    };
</script>
