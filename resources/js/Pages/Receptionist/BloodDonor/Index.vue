    ...
    <AppLayout title="Blood Donor">
        ...
        <div class="mb-4 flex justify-between">
            ...
            <div class="flex items-center space-x-2 sm:space-x-3 ml-auto">
                ...
                <!-- Clear Filter -->
                <ClearFilter v-if="filter.keyword || filter.gender || filter.blood_group"  :href="route('receptionist.bloodDonor.index')"/>
        ...
        <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-100" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
            <div v-if="showFilter" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-5 mb-4 bg-white rounded-lg shadow-xs  items-center p-4">
                ...
                <div>
                    <label for="receptionist_blood_group" class="block text-sm font-medium text-gray-700">{{ __('Blood Group') }}</label>
                    <div class="mt-1">
                        <Multiselect id="receptionist_blood_group" :close-on-select="true" :can-clear="true"
                            :searchable="true" v-model="filterForm.blood_group" :create-option="false"
                            placeholder="Blood Group" :options="bloodGroups"  />
                    </div>
                </div>
                <div>
                    <label for="receptionist_blood_donor_gender" class="block text-sm font-medium text-gray-700">{{ __('Gender') }}</label>
                    <div class="mt-1">
                        <Multiselect id="receptionist_blood_donor_gender" :close-on-select="true" :can-clear="true"
...
<script>
    ...
    export default {
        ...
        methods: {
            changeViewType(type){
                this.viewType = type
                localStorage.setItem("ReceptionistBloodDonorViewType", this.viewType);
            },
            deleteData(id) {
                this.$swal({
                    title: "Are you sure?",
                    ...
                    if (result.isConfirmed) {
                        this.$inertia.delete(route("receptionist.bloodDonor.destroy", id));
                    }
                });
            },
            ...
            filterData(){
                this.loading = true
                this.filterForm.get(route('receptionist.bloodDonor.index'), {
                    onSuccess: () => {
                        this.loading = false
                    },
                    onError: () => {
            ...
            toggleFilter() {
                ...
                localStorage.setItem("receptionistBloodDonor", this.showFilter);
            },
        },
        created() {
            this.showFilter = localStorage.getItem("receptionistBloodDonor") == "true" ? true: false;
            this.viewType = localStorage.getItem("ReceptionistBloodDonorViewType") == "card" ? 'card': 'table';
        },
    };
</script>
