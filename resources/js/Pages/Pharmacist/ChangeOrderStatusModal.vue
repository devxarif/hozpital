...
<div v-if="show" class="bg-gray-500 bg-opacity-80 transition-opacity fixed inset-0 overflow-y-auto overflow-x-hidden top-0 right-0 left-0 z-50 md:inset-0 h-modal md:h-full justify-center items-center flex w-full p-4">
    <div class="relative w-full h-full max-w-xl md:h-auto" v-click-outside="()=> $emit('close-modal')">
        <div class="relative bg-white rounded-lg shadow ">
            ...
            <div class="px-6 py-6 lg:px-8">
                <h3 class="mb-4 text-xl font-medium text-gray-900 ">Change Order Status</h3>
                <form class="mb-4" @submit.prevent="saveData">
                    <div class="mb-4">
                        <Multiselect id="order_user_role" :close-on-select="true" :can-clear="false"
                            :searchable="true" v-model="form.status" :create-option="false"
                            placeholder="Change Status" :options="[
...
<script>
    export default {
        ...
        methods: {
            saveData() {
                this.form.put(route("admin.pharmacy.order.status"), {
                    onSuccess: () => {
                        this.form.reset(),
