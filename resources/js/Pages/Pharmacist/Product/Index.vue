    ...
    <AppLayout>
        ...
        <div class="mb-4 flex justify-between">
            ...
            <div class="flex items-center space-x-2 sm:space-x-3 ml-auto">
                <BaseButton v-if="filter.keyword && filter.keyword.length" as="link" :href="route('pharmacist.product.index')" class="text-white bg-red-600 hover:bg-red-700 px-3 py-2">
        ...
        <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-100" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
            <div v-if="showFilter" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-5 mb-4 bg-white rounded-lg shadow-xs dark:bg-gray-800 items-center p-4">
                ...
                <div>
                    <label for="pharmacist_product_category" class="block text-sm font-medium text-gray-700">{{ __('Product Category') }}</label>
                    <div class="mt-1">
...
export default {
    ...
    methods: {
        deleteData(id) {
            this.$swal({
                ...
                if (result.isConfirmed) {
                    this.$inertia.delete(route("pharmacist.product.destroy", id));
                }
            });
        },
        async changeTab(tab) {
        ...
        toggleFilter() {
            ...
            localStorage.setItem("pharmacistProduct", this.showFilter);
        },
    },
    created() {
        this.showFilter = localStorage.getItem("pharmacistProduct") == "true" ? true: false;
    },
};
</script>
