    ...
    <AppLayout>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="#"
                            class="text-gray-700 hover:text-gray-900 ml-1 md:ml-2 text-sm font-medium">Prescription</a>
                    </div>
                </li>
            </ol>
        </nav>
        <div class="mb-4 flex justify-between">
            ...
            <div class="flex items-center space-x-2 sm:space-x-3 ml-auto">
                <BaseButton v-if="filter.keyword && filter.keyword.length" as="link" :href="route('doctor.prescription.index')" class="text-white bg-red-600 hover:bg-red-700 px-3 py-2">
                    ...
                    <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                        <MenuItems class="absolute right-0 z-10 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                            ...
                            <MenuItem v-slot="{ active }">
                                <a href="javascript:void(0)" @click.prevent="editData(prescription)" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
        ...
        <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-100" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
            <div v-if="showFilter" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-5 mb-4 bg-white rounded-lg shadow-xs dark:bg-gray-800 items-center p-4">
                <div>
                    ...
                    <div class="mt-1">
                        <input v-model="filterForm.keyword" type="text" id="keyword" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm py-2.5" placeholder="Prescription name">
                    </div>
                </div>
                <div>
       ...
       <template v-else-if="!loading && prescriptions && prescriptions.data.length">
           <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-2">
               <span v-for="prescription in prescriptions.data" :key="prescription.id" class="block p-6 bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                   <div class="flex flex-wrap justify-between items-start">
                        <h2 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ prescription?.patient?.user?.name ?? 'N/A' }}</h2>
                       ...
                       <Menu as="div" class="relative inline-block text-left">
                           ...
                           <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                               <MenuItems class="absolute right-0 z-10 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                                   ...
                                   <MenuItem v-slot="{ active }">
                                       <a href="javascript:void(0)" @click.prevent="deleteData(prescription.id)" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
       ...
       <NothingFound v-else>
        ...
        <CreatePrescription :show="showCreateDrawer" @close-drawer="showCreateDrawer = false"/>
        <EditPrescription :show="showEditDrawer" @close-drawer="showEditDrawer = false" :prescription="editPrescription"/>
    </AppLayout>
</template>

<script>
import CreatePrescription from "./Create.vue";
import EditPrescription from "./Edit.vue";
...
export default {
    ...
    methods: {
        deleteData(id) {
            this.$swal({
                ...
                if (result.isConfirmed) {
                    this.$inertia.delete(route("doctor.prescription.destroy", id));
                }
            });
        },
        ...
        filterData(){
            this.loading = true
            this.filterForm.get(route('doctor.prescription.index'), {
                onSuccess: () => {
                    this.loading = false
                },
                onError: () => {
        ...
        toggleFilter() {
            ...
            localStorage.setItem("doctorPrescription", this.showFilter);
        },
    },
    created() {
        this.showFilter = localStorage.getItem("doctorPrescription") == "true" ? true: false;
    },
};
</script>
