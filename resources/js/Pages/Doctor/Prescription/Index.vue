    ...
    <AppLayout>
       ...
       <template v-else-if="!loading && prescriptions && prescriptions.data.length">
           <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-2">
               <span v-for="prescription in prescriptions.data" :key="prescription.id" class="block p-6 bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100   ">
                   <div class="flex flex-wrap justify-between items-start">
                        <h2 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 ">{{ prescription?.patient?.user?.name ?? 'N/A' }}</h2>
