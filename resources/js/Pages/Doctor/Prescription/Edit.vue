                    ...
                    <div class="pointer-events-auto w-screen max-w-xs lg:max-w-2xl">
                        <div class="flex h-full flex-col bg-white shadow-xl">
                            <div class="flex-1 overflow-y-auto py-6 px-4 sm:px-6">
                                ...
                                <div class="mb-4">
                                    <Label :name="__('Name')" id="prescription_name" :hasError="form.errors.name"/>
                                    <BaseInput v-model="form.name" placeholder="Name" id="prescription_name" :hasError="form.errors.name"/>
                                </div>
                                <div class="mb-4">
                                    <Label :name="__('Note')" id="prescription_note" :hasError="form.errors.note" :required="false"/>
                                    <BaseTextarea v-model="form.note" placeholder="Note" id="prescription_note" :hasError="form.errors.note"/>
                                </div>
...
export default {
    ...
    watch: {
        prescription: {
            handler() {
                this.form.name = this.prescription.name
