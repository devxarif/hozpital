    ...
    <AppLayout title="Patient">
        ...
        <div class="mb-4 flex justify-between">
            ...
            <div class="flex items-center space-x-2 sm:space-x-3 ml-auto">
                ...
                <BaseButton v-if="filter.keyword && filter.keyword.length" as="link" :href="route('doctor.pharmacist.index')" class="text-white bg-red-600 hover:bg-red-700 px-3 py-2">
                        ...
                        <MenuItems class="absolute right-0 z-10 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                            <div class="py-1 text-sm">
                                <MenuItem v-slot="{ active }">
                                    <a :href="route('doctor.patient.export', 'pdf')" target="_blank" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
                                        <font-awesome-icon icon="fa-solid fa-file-pdf" class="h-6 w-6 mr-2 text-red-500 group-hover:text-red-500"/>
                                        As PDF
                                    </a>
                                </MenuItem>
                                <MenuItem v-slot="{ active }">
                                    <a :href="route('doctor.patient.export', 'csv')" target="_blank" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
                                        <font-awesome-icon icon="fa-solid fa-file-csv" class="h-6 w-6 mr-2 text-blue-500 group-hover:text-blue-500"/>
                                        As CSV
                                    </a>
                                </MenuItem>
                                <MenuItem v-slot="{ active }">
                                    <a :href="route('doctor.patient.export', 'xlsx')" target="_blank" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
...
<script>
    ...
    export default {
        ...
        methods: {
            changeViewType(type){
                this.viewType = type
                localStorage.setItem("doctorPatientView", this.viewType);
            },
            deleteData(id) {
                this.$swal({
                    title: "Are you sure?",
                    ...
                    if (result.isConfirmed) {
                        this.$inertia.delete(route("doctor.patient.destroy", id));
                    }
                });
            },
            editData(data){
            ...
            filterData(){
                this.loading = true
                this.filterForm.get(route('doctor.patient.index'), {
                    onSuccess: () => {
                        this.loading = false
                    },
                    onError: () => {
            ...
            toggleFilter() {
                ...
                localStorage.setItem("doctorPatient", this.showFilter);
            },
        },
        created() {
            this.showFilter = localStorage.getItem("doctorPatient") == "true" ? true: false;
            this.viewType = localStorage.getItem("doctorPatientView") == "card" ? 'card': 'table';
        },
    };
</script>
