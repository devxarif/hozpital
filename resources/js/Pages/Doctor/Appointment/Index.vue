<template>
    ...
    <AppLayout>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="#"
                            class="text-gray-700 hover:text-gray-900 ml-1 md:ml-2 text-sm font-medium">Appointment</a>
                    </div>
                </li>
            </ol>
        </nav>
            ...
            <div class="hidden sm:block">
                <div class="border-b border-gray-200">
                    <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                        <button type="button" @click="changeTab('all')" :class="['whitespace-nowrap flex py-4 px-1 border-b-2 font-medium text-sm focus:outline-none', filterForm.status == 'all' ? 'border-blue-500 text-blue-600':'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-200' ]">
                            All
                        ...
                        <button type="button" @click="changeTab('completed')" :class="['whitespace-nowrap flex py-4 px-1 border-b-2 font-medium text-sm focus:outline-none', filterForm.status == 'completed' ? 'border-blue-500 text-blue-600':'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-200' ]">
                            Completed
                            <span class="hidden ml-3 py-0.5 px-2.5 rounded-full text-xs font-medium md:inline-block" :class="filterForm.status == 'completed' ? 'bg-blue-100 text-blue-600':'bg-gray-100 text-gray-900'">
                                {{ count_request.completed }}
                            </span>
                        </button>
                        <button type="button" @click="changeTab('cancelled')" :class="['whitespace-nowrap flex py-4 px-1 border-b-2 font-medium text-sm focus:outline-none', filterForm.status == 'cancelled' ? 'border-blue-500 text-blue-600':'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-200' ]">
                            Cancelled
                            <span class="hidden ml-3 py-0.5 px-2.5 rounded-full text-xs font-medium md:inline-block" :class="filterForm.status == 'cancelled' ? 'bg-blue-100 text-blue-600':'bg-gray-100 text-gray-900'">
                                {{ count_request.cancelled }}
                            </span>
                        </button>
                    </nav>
                </div>
       ...
       <template v-else-if="!loading && appointments && appointments.data.length && viewType == 'card'">
               ...
               <span v-for="appointment in appointments.data" :key="appointment.id" class="block p-6 bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100   ">
                   <div class="flex flex-wrap justify-between items-start">
                       <div class="relative mb-5" v-if="appointment.patient && appointment.patient.user">
                           <span>
                               <img class="w-16 h-16 rounded object-cover" alt="image" :src="appointment.patient.user.avatar_url">
                           </span>
                       </div>
                       <Menu as="div" class="relative inline-block text-left">
                            <div>
                            ...
                            <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                                <MenuItems class="absolute right-0 z-10 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                                    <div class="py-1 text-sm">
                                        <MenuItem v-slot="{ active }" v-if="appointment.status == 'pending'">
                                            <a href="javascript:void(0)" @click.prevent="markStatus(appointment.id, 'approved')" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
                                                <font-awesome-icon icon="fa-solid fa-check" class="mr-3 h-5 w-5 text-green-500 group-hover:text-green-500"/>
                                                Mark as approved
                                            </a>
                                        </MenuItem>
                                        <MenuItem v-slot="{ active }" v-if="appointment.status == 'approved'">
                                            <a href="javascript:void(0)" @click.prevent="markStatus(appointment.id, 'completed')" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
                                                <font-awesome-icon icon="fa-solid fa-check" class="mr-3 h-5 w-5 text-green-500 group-hover:text-green-500"/>
                                                Mark as completed
                                            </a>
                                        </MenuItem>
                                        <MenuItem v-slot="{ active }" v-if="appointment.status == 'pending' || appointment.status == 'approved'">
                                            <a href="javascript:void(0)" @click.prevent="markStatus(appointment.id, 'cancelled')" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
                                                <font-awesome-icon icon="fa-solid fa-xmark" class="mr-3 h-5 w-5 text-red-500 group-hover:text-red-500"/>
                                                Mark as cancelled
                                            </a>
                                        </MenuItem>
                                        <MenuItem v-slot="{ active }">
                                            <a href="javascript:void(0)" @click.prevent="editData(appointment)" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
        ...
        <!-- Table View  -->
        <div class="flex flex-col mb-5 shadow-lg" v-else-if="!loading && appointments && appointments.data.length && viewType == 'table'">
                    ...
                    <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                        <table class="w-full divide-y divide-gray-300 table-fixed">
                            ...
                            <tr class="divide-x divide-gray-200">
                                <th class="py-3.5 pl-4 pr-4 text-left text-sm font-semibold text-gray-900 sm:pl-6">Patient</th>
                                <th class="px-4 py-3.5 text-left text-sm font-semibold text-gray-900 break-words">Problem</th>
                                <th width="20%" class="px-4 py-3.5 text-left text-sm font-semibold text-gray-900 break-words">Time</th>
                                <th width="10%" class="px-4 py-3.5 text-left text-sm font-semibold text-gray-900 break-words">Status</th>
                                <th width="80px" class="py-3.5 pl-4 pr-4 text-left text-sm font-semibold text-gray-900 sm:pr-6">Action</th>
                            </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200 bg-white">
                                <tr v-for="appointment in appointments.data" :key="appointment.id" class="divide-x divide-gray-200">
                                    <td class="py-4 pl-4 pr-4 text-sm font-medium text-gray-900 sm:pl-6">
                                        <div class="flex items-center" v-if="appointment.patient && appointment.patient.user">
                                            <div class="h-10 w-10 flex-shrink-0">
                                                <img class="h-10 w-10 rounded-md" :src="appointment.patient.user.avatar_url" alt="">
                                            </div>
                                            <div class="ml-4">
                                                <div class="font-medium text-gray-900">{{ appointment.patient.user.name }}</div>
                                                <div class="text-gray-500">{{ appointment.patient.user.email }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="p-4 text-sm text-gray-500 break-all">
                                        <p class="text-md font-extrabold">Sat 25 Feb 2023</p>
                                        <span class="block">{{ appointment.slot }}</span>
                                    </td>
                                    <td class="p-4 text-sm text-gray-500 break-all">
                                        <div class="mt-5">
                                            <span :class="getAppointmentStatusClass(appointment.status)" class="text-white text-sm font-medium mr-2 px-3 py-2 rounded-full   capitalize">
       ...
       <NothingFound v-else>
        ...
        <EditDepartment :show="showEditDrawer" @close-drawer="showEditDrawer = false" :appointments="editAppointment"/> -->
    </AppLayout>
</template>
...
import { faCheck, faHourglassStart, faXmark } from '@fortawesome/free-solid-svg-icons'
library.add(faCheck, faHourglassStart, faXmark)
...
export default {
    ...
    methods: {
        ...
        markStatus(id, status){
            this.$swal({
                ...
                if (result.isConfirmed) {
                    this.$inertia.put(route("doctor.appointment.status", id), {
                        status: status
                    });
                }
            });
        ...
        toggleFilter() {
            ...
            localStorage.setItem("adminAppointment", this.showFilter);
        },
    },
    created() {
        this.showFilter = localStorage.getItem("adminAppointment") == "true" ? true: false;
        this.viewType = localStorage.getItem("adminAppointmentView") == "card" ? 'card': 'table';
    },
};
</script>
