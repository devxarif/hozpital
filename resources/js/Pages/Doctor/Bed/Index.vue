    ...
    <AppLayout title="Bed">
        ...
        <div class="mb-4 flex justify-between">
            ...
            <div class="flex items-center space-x-2 sm:space-x-3 ml-auto">
                <!-- Clear Filter -->
                <ClearFilter v-if="filter.keyword || filter.bed_type || filter.bed_floor || filter.status" :href="route('doctor.bed.index')"/>
        ...
        <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-100" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
            <div v-if="showFilter" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-5 mb-4 bg-white rounded-lg shadow-xs  items-center p-4">
                ...
                <div>
                    <label for="doctor_bed_type" class="block text-sm font-medium text-gray-700">{{ __('Bed Type') }}</label>
                    <div class="mt-1">
                        <Multiselect id="doctor_bed_type" :close-on-select="true" :can-clear="true"
                ...
                <div>
                    <label for="doctor_bed_floor" class="block text-sm font-medium text-gray-700">{{ __('Floor') }}</label>
                    <div class="mt-1">
                        <Multiselect id="doctor_bed_floor" :close-on-select="true" :can-clear="true"
                ...
                <div>
                    <label for="doctor_bed_status" class="block text-sm font-medium text-gray-700">{{ __('Status') }}</label>
                    <div class="mt-1">
                        <Multiselect id="doctor_bed_status" :close-on-select="true" :can-clear="true"
...
<script>
    ...
    export default {
        ...
        methods: {
            deleteData(id) {
                this.$swal({
                    ...
                    if (result.isConfirmed) {
                        this.$inertia.delete(route("doctor.bed.destroy", id));
            ...
            filterData(){
                this.loading = true
                this.filterForm.get(route('doctor.bed.index'), {
                    onSuccess: () => {
                        this.loading = false
                    },
                    onError: () => {
            ...
            toggleFilter() {
                ...
                localStorage.setItem("doctorBed", this.showFilter);
            }
        },
        created() {
            this.showFilter = localStorage.getItem("doctorBed") == "true" ? true: false;
        },
    };
</script>
