<template>
    <AppLayout title="Bed Type">
        ...
        <template v-if="bed_types && bed_types.data.length">
            ...
            <span v-for="bed_type in bed_types.data" :key="bed_type.id" class="block p-6 bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100   ">
                <div class="flex flex-wrap justify-between items-start">
                    <h2 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 ">{{ bed_type.name }}</h2>
            ...
            <Pagination :data="bed_types" v-if="bed_types && bed_types.data.length && bed_types.total > app_setting.rows_per_page" class="mt-5"/>
        </template>
...
<script>
    export default {
        ...
        methods: {
            deleteData(id) {
                this.$swal({
                    ...
                    if (result.isConfirmed) {
                        this.$inertia.delete(route("nurse.bedType.destroy", id));
                    }
                });
            },
            editData(bed){
            ...
            filterData(){
                this.loading = true
                this.filterForm.get(route('nurse.bedType.index'), {
                    onSuccess: () => {
                        this.loading = false
                    },
                    onError: () => {
            ...
            toggleFilter() {
                ...
                localStorage.setItem("nurseBedType", this.showFilter);
            },
        },
        created() {
            this.showFilter = localStorage.getItem("nurseBedType") == "true" ? true: false;
        },
    };
</script>
