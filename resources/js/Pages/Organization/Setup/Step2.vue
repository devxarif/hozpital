    ...
    <div class="email-login mt-4">
        ...
        <form @submit.prevent="saveData">
            <div class="row my-1 justify-content-center" v-for="(name, index) in form.names" :key="index">
                <div class="col-lg-8">
                    <input v-model="form.names[index]" :class="{'border-danger':form.errors.names}" type="text" :placeholder="__('Team Name')" id="organization_name" class="d-block">
...
<script>
    ...
    methods: {
        async getTeams(){
            let response = await axios.get(route('fetch.organization.teams'))
        ...
        saveData(){
            this.form.post(route('organization.account.setup.step2'), {
                onSuccess: () => {
                    this.form.reset()
                    this.getTeams()
                }
        ...
        deleteData(id){
            this.$swal({
                ...
                if (result.isConfirmed) {
                    this.$inertia.delete(route("delete.organization.teams", id), {
