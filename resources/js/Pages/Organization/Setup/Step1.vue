       ...
       <form @submit.prevent="saveData">
            <div class="row">
                <div class="col-lg-6">
                    ...
                    <input v-model="form.organization_name" :class="{'border-danger':form.errors.organization_name}" type="text" :placeholder="__('Company Name')" id="organization_name" class="d-block">
                    <AuthErrorMessage :name="form.errors.organization_name" />
                </div>
                <div class="col-lg-6">
                    <AuthLabel name="Company Email" />
                    <input v-model="form.organization_email" :class="{'border-danger':form.errors.organization_email}" type="text"
                    :placeholder="__('Email')" id="organization_email" class="d-block">
                    <AuthErrorMessage :name="form.errors.organization_email" />
                </div>
                <div class="col-lg-6">
...
<script>
    ...
    methods: {
        saveData(){
            try {
                this.form.errors = [];
                this.form.post(route('organization.account.setup.step1'), {
                    onSuccess:() => {
                        this.$emit('step',2)
                    }
                })
