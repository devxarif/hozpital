        ...
        <div class="col-12">
            <div class="card mt-3">
                <div class="card-header">
                    <div class="d-flex justify-content-between">
                        <h3 class="card-title">{{ __('Holiday Request List') }}</h3>
                         <Link :href="route('organization.holidays.index')" class="btn btn-primary">
                ...
                <div class="card-body table-responsive p-0">
                            ...
                            <template v-if="holidays && holidays.data.length">
                                <tr v-for="(holiday,index) in holidays.data" :key="index">
                                    <td v-if="holiday.employee.user">
                                        ...
                                        <Link :href="route('organization.employees.show',holiday.employee.user_id)">{{ holiday.employee.user.name }}</Link>
                                    </td>
                                    <td >
                                        {{ holiday.title }}
                                    </td>
                    ...
                    <!-- Pagination  -->
                    <Pagination :data="holidays.links" />
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        acceptRequest(id) {
            this.$inertia.post(route("organization.request.holidays.accept"), {
                id,
            });
        },
        rejectRequest(id) {
            this.$swal({
                ...
                if (result.isConfirmed) {
                    this.$inertia.delete(
                        route("organization.request.holidays.reject", id)
                    );
                }
            });
        },
