                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        <div v-click-outside="()=> showHolidayDropdown = false">
                            <div class="btn-group" :class="{'show':showHolidayDropdown}" role="group">
                                ...
                                <Link :href="route('organization.request.holidays')" @click="showEmployeeCreateModal = true" class="dropdown-item" v-tooltip="'Show all requested holidays'">
...
export default {
    ...
    methods: {
        saveData() {
            if (!this.editMode) {
                this.form.post(
                    route("organization.holidays.store"),
                    {
                        onSuccess: () => {
                            this.showModal = true;
                            this.form.reset();
                ...
                this.form.put(
                    route("organization.holidays.update", this.holiday_id),
                    {
                        onSuccess: () => {
        ...
        deleteData(holiday_id) {
            this.$swal({
                ...
                if (result.isConfirmed) {
                    this.form.delete(
                        route("organization.holidays.destroy", holiday_id)
                    );
                }
            });
        },
        ...
        deleteHolidays() {
            this.$swal({
                ...
                if (result.isConfirmed) {
                    this.$inertia.delete(route("organization.holidays.all.destroy"));
                }
            });
        },
        ...
        importHolidays(type) {
            ...
            this.importForm.post(
                route("organization.holidays.import"),
                {
                    onSuccess: () => {
