                ...
                <div class="overflow-x-auto -mx-4 -my-6 sm:-m-6">
                    <div class="align-middle inline-block min-w-full">
                        <div class="overflow-hidden border-b border-gray-200 sm:rounded-lg">
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="divide-y divide-gray-200">
                                    <template v-if="teams && teams.data.length">
                                        <tr class="relative hover:bg-slate-50" v-for="(team,index) in teams.data" :key="index">
                                            ...
                                            <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                <ul class="list-inline" v-if="team.employees && team.employees.length">
                                                    <li class="list-inline-item" v-for="(employee, key) in team.employees" :key="employee.id" v-tooltip="employee.user.name">
                                                        <Link :href="route('organization.employees.show',employee.user_id)">
                            ...
                            <Pagination :data="teams" v-if="teams && teams.data.length" />
                        </div>
...
export default {
    ...
    methods: {
        ...
        createData() {
            this.form.post(route("organization.teams.store"), {
        ...
        updateData() {
            this.form.put(route("organization.teams.update", this.selectedId), {
                onSuccess: () => {
                    this.isEditMode = false;
                    this.form.reset();
                },
        ...
        deleteData(id) {
            this.$swal({
                ...
                if (result.isConfirmed) {
                    this.$inertia.delete(route("organization.teams.destroy", id));
                }
            });
        },
        addMore() {
