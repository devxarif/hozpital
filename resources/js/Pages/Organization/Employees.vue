                                    ...
                                    <template v-if="employees && employees.length">
                                        <div class="col mb-4" v-for="employee in employees" :key="employee.id">
                                            <div class="card h-100" v-if="employee.user">
                                                <img :src="employee.user.avatar_url" class="card-image-top">
                                                <div class="card-body">
                                                    ...
                                                    <Link :href="route('organization.employees.show',employee.user_id)" v-tooltip="'Details'" class="btn btn-sm team-member-show">
                                                        <EyeIcon/>
                                                    </Link>
    ...
    methods: {
        async changeTab(tab) {
            ...
            let response = await axios.get(
                route("organization.teams.employees", {
                    team: tab,
                })
            );
        ...
        deleteData(id) {
            this.$swal({
                ...
                if (result.isConfirmed) {
                    this.$inertia.delete(route("organization.employees.destroy", id), {
                        onSuccess: () => {
