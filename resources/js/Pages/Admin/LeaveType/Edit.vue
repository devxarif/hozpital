...
export default {
    ...
    data() {
        return {
            form: this.$inertia.form({
                ...
                color: this.leave_type.color ?? "#FF1900",
                balance: this.leave_type.balance,
                auto_approve: this.leave_type.auto_approve ? true:false,
                status: this.leave_type.status ? true:false,
                _method: 'PUT'
            }),
    ...
    watch: {
        leave_type: {
            handler() {
                this.form.name = this.leave_type.name,
                this.form.color = this.leave_type.color ?? "#FF1900",
                this.form.balance = this.leave_type.balance,
                this.form.auto_approve = this.leave_type.auto_approve ? true:false,
                this.form.status = this.leave_type.status ? true:false
            },
            deep: true,
        },
    ...
    methods: {
        saveData() {
            this.form.post(route("admin.leaveType.update", this.leave_type.id), {
                onSuccess: () => {
                    this.form.reset(),
