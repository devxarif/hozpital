                        ...
                        <div class="flex-1 overflow-y-auto py-6 px-4 sm:px-6">
                            ...
                            <form class="mb-4" @submit.prevent="saveData">
                                <div class="grid grid-cols-2 gap-2">
                                    <div class="mb-4">
                                        <Label name="Name" id="nurse_name" :hasError="form.errors.name"/>
                                        <BaseInput v-model="form.name" placeholder="Name" id="nurse_name" :hasError="form.errors.name"/>
                                    </div>
                                    <div class="mb-4">
                                        <Label name="Email" id="nurse_email" :hasError="form.errors.email"/>
...
export default {
    ...
    watch: {
        nurse: {
            handler() {
                this.form.name = this.nurse.user?.name ?? ''
                this.form.email = this.nurse.user?.email ?? ''
                this.previewImage = this.nurse.avatar
            },
            deep: true,
        },
    },
    data() {
        return {
            ...
            previewImage: this.nurse.avatar,
    ...
    methods: {
        ...
        saveData() {
            this.form.post(route("admin.nurse.update", this.nurse.id), {
                onSuccess: () => {
                    this.form.reset(),
                    this.removeImage()
