                        ...
                        <div class="flex-1 overflow-y-auto py-6 px-4 sm:px-6">
                            ...
                            <form class="mb-4" @submit.prevent="saveData">
                                <div class="grid grid-cols-2 gap-2">
                                    <div class="mb-4">
                                        <Label :name="__('Name')" id="receptionist_name" :hasError="form.errors.name"/>
                                        <BaseInput v-model="form.name" placeholder="Name" id="receptionist_name" :hasError="form.errors.name"/>
                                    </div>
                                    <div class="mb-4">
                                        <Label :name="__('Email')" id="receptionist_email" :hasError="form.errors.email"/>
                                        <BaseInput v-model="form.email" placeholder="Email Address" id="receptionist_email" :hasError="form.errors.email"/>
                                    </div>
                                </div>
                                <div class="mb-4">
                                    ...
                                    <div class="relative">
                                        <BaseInput v-model="form.password" placeholder="Password" id="receptionist_password" :hasError="form.errors.password" :type="passwordFieldType"/>
...
export default {
    ...
    watch: {
        receptionist: {
            handler() {
                this.form.name = this.receptionist.user?.name ?? ''
                this.form.email = this.receptionist.user?.email ?? ''
                this.previewImage = this.receptionist.avatar
            },
            deep: true,
        },
    },
    data() {
        return {
            form: this.$inertia.form({
                name: this.receptionist.user?.name ?? '',
                email: this.receptionist.user?.email ?? '',
                password: "",
                avatar: '',
                _method: 'PUT'
            }),

            previewImage: this.receptionist.avatar,
    ...
    methods: {
        ...
        saveData() {
            this.form.post(route("admin.receptionist.update", this.receptionist.id), {
                onSuccess: () => {
                    this.form.reset(),
                    this.removeImage()
