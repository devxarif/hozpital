<template>
    <AppLayout title="Doctor">
        ...
        <!-- Header Part  -->
        <Breadcrumb>
            <BreadcrumbLink title="Pharmacist"/>
        </Breadcrumb>
       ...
       <template v-else-if="!loading && pharmacists && pharmacists.data.length && viewType == 'card'">
            <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
                <span v-for="pharmacist in pharmacists.data" :key="pharmacist.id" class="block p-6 bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100   ">
                    ...
                    <h2 class="text-2xl font-bold tracking-tight text-gray-900 ">{{ pharmacist.user?.name ?? '-' }}</h2>
        ...
        <!-- Table View  -->
        <BaseTable v-else-if="!loading && pharmacists && pharmacists.data.length && viewType == 'table'" :items="pharmacists">
