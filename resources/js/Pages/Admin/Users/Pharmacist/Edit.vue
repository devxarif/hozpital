                        ...
                        <div class="flex-1 overflow-y-auto py-6 px-4 sm:px-6">
                            ...
                            <form class="mb-4" @submit.prevent="saveData">
                                <div class="grid grid-cols-2 gap-2">
                                    <div class="mb-4">
                                        <Label name="Name" id="pharmacist_name" :hasError="form.errors.name"/>
                                        <BaseInput v-model="form.name" placeholder="Name" id="pharmacist_name" :hasError="form.errors.name"/>
                                    </div>
                                    <div class="mb-4">
                                        <Label name="Email" id="pharmacist_email" :hasError="form.errors.email"/>
...
export default {
    ...
    watch: {
        pharmacist: {
            handler() {
                this.form.name = this.pharmacist.user?.name ?? ''
                this.form.email = this.pharmacist.user?.email ?? ''
                this.previewImage = this.pharmacist.avatar
            },
            deep: true,
        },
    },
    data() {
        return {
            form: this.$inertia.form({
                ...
                email: this.pharmacist.user?.email ?? '',
                password: "",
                avatar: '',
                _method: 'PUT'
            }),

            previewImage: this.pharmacist.avatar,
    ...
    methods: {
        ...
        saveData() {
            this.form.post(route("admin.pharmacist.update", this.pharmacist.id), {
                onSuccess: () => {
                    this.form.reset(),
                    this.removeImage()
