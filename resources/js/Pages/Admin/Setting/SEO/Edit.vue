                            ...
                            <div class="flex-1 overflow-y-auto py-6 px-4 sm:px-6">
                                <div class="flex items-start justify-between rtl:flex-row-reverse mb-5">
                                    <h2 class="text-2xl tracking-wide font-bold text-gray-900">
                                        {{ __('SEO Content Update') }} - {{ seo.page_name }}
                                    </h2>
                                ...
                                <form class="mb-4" @submit.prevent="saveData">
                                    <div class="mb-4">
                                        <Label name="Meta Title" id="meta_title" :hasError="form.errors.title"/>
                                        <BaseInput v-model="form.title" placeholder="Title" id="meta_title" :hasError="form.errors.title"/>
                                    </div>
                                    <div class="mb-4">
                                        <Label name="Meta Keywords" id="meta_keywords" :hasError="form.errors.keywords"/>
                                        <BaseTextarea v-model="form.keywords" placeholder="Keywords" id="meta_keywords" :hasError="form.errors.keywords" rows="1"/>
                                        <small>Keyword separate by ,</small>
                                    </div>
                                    <div class="mb-4">
                                        <Label name="Meta Description" id="meta_description" :hasError="form.errors.description"/>
                                        <BaseTextarea v-model="form.description" placeholder="Description" id="meta_description" :hasError="form.errors.description"/>
                                    </div>
                                    <div class="mb-4">
...
export default {
    ...
    watch: {
        seo: {
            handler() {
                ...
                this.form.keywords = this.seo.keywords
                this.form.description = this.seo.description
                this.previewImage = this.seo.image
            },
            deep: true,
        },
    },
    methods: {
        ...
        saveData() {
            this.form.post(route("admin.settings.seo.update", this.seo.id), {
                onSuccess: () => {
                    this.form.reset(),
                    this.removeImage()
