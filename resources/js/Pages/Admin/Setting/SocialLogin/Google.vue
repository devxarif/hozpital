    ...
    <form @submit.prevent="updateData" class="space-y-8 divide-y divide-gray-200">
        <div class="space-y-8 divide-y divide-gray-200 sm:space-y-5">
            <div class="space-y-6 sm:space-y-5">
                <div>
                    <h2 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 ">Google Login</h2>
...
export default {
    ...
    data() {
        return {
            form: this.$inertia.form({
                ...
                google_secret: this.data.google_secret,
                google_active: this.data.google_active ? true: false,
                provider: "google",
            }),
        };
    },
    methods: {
        statusChange(event) {
            if (event.target.checked == true) {
                this.form.google_active = true;
            } else {
                this.form.google_active = false;
            }
        },
        updateData() {
    ...
    watch: {
        data: {
            handler() {
                this.form.google_id = this.data.google_id;
                this.form.google_secret = this.data.google_secret;
                this.form.google_active = this.data.google_active ? true: false;
            },
            deep: true,
        },
    },
