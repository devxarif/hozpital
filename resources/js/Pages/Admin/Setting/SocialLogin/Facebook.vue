    ...
    <form @submit.prevent="updateData" class="space-y-8 divide-y divide-gray-200">
        <div class="space-y-8 divide-y divide-gray-200 sm:space-y-5">
            <div class="space-y-6 sm:space-y-5">
                <div>
                    <h2 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 ">Facebook Login</h2>
                ...
                <div class="space-y-6 sm:space-y-5">
                    ...
                    <div class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5">
                        <Label name="Status" id="facebook_status" :hasError="form.errors.facebook_active"/>
                        <div class="mt-1 sm:col-span-2 sm:mt-0">
                            <label for="checked-toggle" class="inline-flex relative items-center cursor-pointer">
                                <input @change="statusChange" v-model="form.facebook_active" type="checkbox" id="checked-toggle" class="sr-only peer" :checked="form.facebook_active">
                                <div
...
export default {
    ...
    data() {
        return {
            form: this.$inertia.form({
                ...
                facebook_secret: this.data.facebook_secret,
                facebook_active: this.data.facebook_active ? true: false,
                provider: "facebook",
            }),
        };
    },
    methods: {
        statusChange(event) {
            if (event.target.checked == true) {
                this.form.facebook_active = true;
            } else {
                this.form.facebook_active = false;
            }
        },
        updateData() {
    ...
    watch: {
        data: {
            handler() {
                this.form.facebook_id = this.data.facebook_id;
                this.form.facebook_secret = this.data.facebook_secret;
                this.form.facebook_active = this.data.facebook_active ? true: false;
            },
            deep: true,
        },
    },
