<template>
    <SettingLayout title="Cookies Alert">
                ...
                <form @submit.prevent="updateData" class="space-y-8 divide-y divide-gray-200">
                    <div class="space-y-8 divide-y divide-gray-200 sm:space-y-5">
                        <div class="space-y-6 sm:space-y-5">
                            <div>
                                <h2 class="mb-2 text-2xl font-bold tracking-tight text-gray-900">Cookies Alert</h2>
                                ...
                                <div class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5">
                                    ...
                                    <div class="mt-1 sm:col-span-2 sm:mt-0">
                                        <div class="max-w-lg rounded-md shadow-sm">
                                            ...
                                            <ErrorMessage :name="form.errors.cookies_alert_message" />
                                        </div>
                                    </div>
                                </div>
                                <div class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5">
    ...
    data() {
        return {
            form: this.$inertia.form({
                cookies_alert_message: this.$page.props.setting.cookies_alert_message,
                cookies_alert_button_text: this.$page.props.setting.cookies_alert_button_text,
                cookies_alert_active: this.$page.props.setting.cookies_alert_active ? true: false,
            }),
        };
    },
    methods: {
        statusChange(event) {
            if (event.target.checked == true) {
                this.form.cookies_alert_active = true;
            } else {
                this.form.cookies_alert_active = false;
            }
        },
        updateData() {
            this.form.put(route("admin.settings.cookiesAlert.update"), {
                preserveScroll: true
            });
        },
    }
