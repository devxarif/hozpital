                    ...
                    <div class="pointer-events-auto w-screen max-w-xs lg:max-w-2xl">
                        <div class="flex h-full flex-col bg-white shadow-xl">
                            <div class="flex-1 overflow-y-auto py-6 px-4 sm:px-6">
                                ...
                                <div class="mb-4">
                                    <Label :name="__('Name')" id="expense_category_name" :hasError="form.errors.name"/>
                                    <BaseInput v-model="form.name" placeholder="Name" id="expense_category_name" :hasError="form.errors.name"/>
                                </div>
                                <div class="mb-4">
                                    <Label :name="__('Description')" id="expense_category_description" :hasError="form.errors.description" :required="false"/>
                                    <BaseTextarea v-model="form.description" placeholder="Description" id="expense_category_description" :hasError="form.errors.description"/>
                                </div>
...
export default {
    ...
    data() {
        return {
            form: this.$inertia.form({
                ...
                description: this.expense_category.description,
                _method: 'PUT'
            }),
        };
    },
    watch: {
        expense_category: {
            handler() {
                this.form.name = this.expense_category.name
                this.form.description = this.expense_category.description
            },
            deep: true,
        },
    },
    methods: {
        saveData() {
            this.form.post(route("admin.expenseCategory.update", this.expense_category.id), {
                onSuccess: () => {
                    this.form.reset(),
