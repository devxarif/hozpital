    ...
    <AppLayout title="Expense">
        ...
        <div class="mb-4 flex justify-between">
            ...
            <div class="flex items-center space-x-2 sm:space-x-3 ml-auto">
                ...
                <!-- Clear Filter -->
                <ClearFilter v-if="filter.keyword && filter.keyword.length"  :href="route('admin.expense.index')" />
       ...
       <!-- Card View  -->
       <template v-else-if="!loading && expenses && expenses.data.length && viewType == 'card'">
                       ...
                       <Menu as="div" class="relative inline-block text-left">
                           ...
                           <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                               <MenuItems class="absolute right-0 z-10 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                                   <div class="py-1 text-sm">
                                   <MenuItem v-slot="{ active }">
                                       <a href="javascript:void(0)" @click.prevent="editData(expense)" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
                                   ...
                                   <MenuItem v-slot="{ active }">
                                       <a href="javascript:void(0)" @click.prevent="deleteData(expense.id)" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
        ...
        <!-- Table View  -->
        <BaseTable v-else-if="!loading && expenses && expenses.data.length && viewType == 'table'" :items="expenses">
            ...
            <template v-slot:body>
                <tr v-for="expense in expenses.data" :key="expense.id" class="divide-x divide-gray-200">
       ...
       <NothingFound v-else>
        ...
        <EditExpense :show="showEditDrawer" @close-drawer="showEditDrawer = false" :expense="editExpense"/>
    </AppLayout>
</template>
...
export default {
    ...
    methods: {
        changeViewType(type){
            this.viewType = type
            localStorage.setItem("adminExpenseViewType", this.viewType);
        },
        deleteData(id) {
            this.$swal({
                title: "Are you sure?",
                ...
                if (result.isConfirmed) {
                    this.$inertia.delete(route("admin.expense.destroy", id));
                }
            });
        },
        editData(expense){
        ...
        filterData(){
            this.loading = true
            this.filterForm.get(route('admin.expense.index'), {
                onSuccess: () => {
                    this.loading = false
                },
                onError: () => {
        ...
        toggleFilter() {
            ...
            localStorage.setItem("adminExpense", this.showFilter);
        },
    },
    created() {
        this.showFilter = localStorage.getItem("adminExpense") == "true" ? true: false;
        this.viewType = localStorage.getItem("adminExpenseViewType") == "card" ? 'card': 'table';
    },
};
</script>
