<template>
    <AppLayout title="Product">
        <!-- Header Part  -->
        <Breadcrumb>
            <BreadcrumbLink title="Product Category"/>
        </Breadcrumb>
       ...
       <CardSkeleton :show="loading" v-if="loading"/>
        ...
        <!-- Card View  -->
       <template v-else-if="!loading && product_categories && product_categories.data.length && viewType == 'card'">
               ...
               <span v-for="produt_category in product_categories.data" :key="produt_category.id" class="block p-6 bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100   ">
                   <div class="flex flex-wrap justify-between items-start">
                       <div class="relative mb-5">
                           <span>
                               <img class="w-16 h-16 rounded object-cover" alt="image" :src="produt_category.image">
                           </span>
                       </div>
                       <Menu as="div" class="relative inline-block text-left">
                           <div>
        ...
        <!-- Table View  -->
        <BaseTable v-else-if="!loading && product_categories && product_categories.data.length && viewType == 'table'" :items="product_categories">
            <template v-slot:head>
                <tr class="divide-x divide-gray-200">
                    <th class="py-3.5 pl-4 pr-4 text-left text-sm font-semibold text-gray-900 sm:pl-6">Name</th>
                    <th class="px-4 py-3.5 text-left text-sm font-semibold text-gray-900 break-words">Description</th>
                    <th width="80px" class="py-3.5 pl-4 pr-4 text-left text-sm font-semibold text-gray-900 sm:pr-6">Action</th>
                </tr>
            </template>
            <template v-slot:body>
                <tr v-for="product_category in product_categories.data" :key="product_category.id" class="divide-x divide-gray-200">
                    <td class="py-4 pl-4 pr-4 text-sm font-medium text-gray-900 sm:pl-6">
                        <div class="flex items-center">
                            <img class="h-16 w-16 rounded-md object-cover" :src="product_category.image" alt="">
                            <div class="ml-4">
                                <div class="font-medium text-gray-900">{{ product_category.name }}</div>
                            </div>
                        </div>
                    </td>
                        ...
                        <Menu as="div" class="inline-block text-left">
                            ...
                            <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                                <MenuItems class="absolute right-0 z-10 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                                    <div class="py-1 text-sm">
                                    <MenuItem v-slot="{ active }">
                                        <a href="javascript:void(0)" @click.prevent="editData(product_category)" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
                                            <font-awesome-icon icon="fa-solid fa-pen-to-square" class="mr-3 h-5 w-5 text-blue-500 group-hover:text-blue-500"/>
                                            Edit
                                        </a>
                                    </MenuItem>
                                    ...
                                    <MenuItem v-slot="{ active }">
                                        <a href="javascript:void(0)" @click.prevent="deleteData(product_category.id)" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
                                            <font-awesome-icon icon="fa-solid fa-trash-can" class="mr-3 h-5 w-5 text-red-500 group-hover:text-red-500"/>
                                            Delete
                                        </a>
                                    </MenuItem>
...
export default {
    ...
    methods: {
        changeViewType(type){
            this.viewType = type
            localStorage.setItem("adminProductCategory", this.viewType);
        },
        deleteData(id) {
            this.$swal({
                title: "Are you sure?",
    ...
    created() {
        ...
        this.viewType = localStorage.getItem("adminProductCategory") == "card" ? 'card': 'table';
    },
};
</script>
