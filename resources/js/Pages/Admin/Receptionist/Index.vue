    ...
    <AppLayout>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="#"
                            class="text-gray-700 hover:text-gray-900 ml-1 md:ml-2 text-sm font-medium">Receptionist</a>
                    </div>
                </li>
            </ol>
        </nav>
        <div class="mb-4 flex justify-between">
            ...
            <div class="flex items-center space-x-2 sm:space-x-3 ml-auto">
                ...
                <BaseButton v-if="filter.keyword && filter.keyword.length" as="link" :href="route('admin.receptionist.index')" class="text-white bg-red-600 hover:bg-red-700 px-3 py-2">
                        ...
                        <MenuItems class="absolute right-0 z-10 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                            <div class="py-1 text-sm">
                                <MenuItem v-slot="{ active }">
                                    <a :href="route('admin.receptionist.export', 'pdf')" target="_blank" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
                                        <font-awesome-icon icon="fa-solid fa-file-pdf" class="h-6 w-6 mr-2 text-red-500 group-hover:text-red-500"/>
                                        As PDF
                                    </a>
                                </MenuItem>
                                <MenuItem v-slot="{ active }">
                                    <a :href="route('admin.receptionist.export', 'csv')" target="_blank" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
                                        <font-awesome-icon icon="fa-solid fa-file-csv" class="h-6 w-6 mr-2 text-blue-500 group-hover:text-blue-500"/>
                                        As CSV
                                    </a>
                                </MenuItem>
                                <MenuItem v-slot="{ active }">
                                    <a :href="route('admin.receptionist.export', 'xlsx')" target="_blank" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
        ...
        <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-100" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
            <div v-if="showFilter" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-5 mb-4 bg-white rounded-lg shadow-xs dark:bg-gray-800 items-center p-4">
                <div>
                    ...
                    <div class="mt-1">
                        <input v-model="filterForm.keyword" type="text" id="keyword" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm py-2.5" placeholder="Receptionist name, email">
                    </div>
                </div>
                <div>
        ...
        <!-- Card View  -->
       <template v-else-if="!loading && receptionists && receptionists.data.length && viewType == 'card'">
            <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
                <span v-for="receptionist in receptionists.data" :key="receptionist.id" class="block p-6 bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                    <div class="flex flex-wrap justify-between items-start">
                        <div class="relative mb-5">
                            <span>
                                <img class="w-16 h-16 rounded object-cover" alt="image" :src="receptionist.avatar">
                            </span>
                        </div>
                        <Menu as="div" class="relative inline-block text-left">
                            <div>
                            ...
                            <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                                <MenuItems class="absolute right-0 z-10 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                                    <div class="py-1 text-sm">
                                    <MenuItem v-slot="{ active }">
                                        <a href="javascript:void(0)" @click.prevent="editData(receptionist)" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
                                    ...
                                    <MenuItem v-slot="{ active }">
                                        <a href="javascript:void(0)" @click.prevent="deleteData(receptionist.id)" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
                    ...
                    <h2 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ receptionist.user?.name ?? '-' }}</h2>
                    <h6 class="my-1 text-sm font-bold tracking-tight text-gray-900 dark:text-white">{{ receptionist.user.email ?? '-' }}</h6>
                </span>
            </div>
        ...
        <!-- Table View  -->
        <div class="flex flex-col mb-5 shadow-lg" v-else-if="!loading && receptionists && receptionists.data.length && viewType == 'table'">
                    ...
                    <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                        <table class="w-full divide-y divide-gray-300 table-fixed">
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white">
                                <tr v-for="receptionist in receptionists.data" :key="receptionist.id" class="divide-x divide-gray-200">
                                    <td class="py-4 pl-4 pr-4 text-sm font-medium text-gray-900 sm:pl-6">
                                        <div class="flex items-center" v-if="receptionist && receptionist.user">
                                            <div class="h-10 w-10 flex-shrink-0">
                                                <img class="h-10 w-10 rounded-md" :src="receptionist.user.avatar_url" alt="">
                                            </div>
                                            <div class="ml-4">
                                                <div class="font-medium text-gray-900">{{ receptionist.user.name }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="p-4 text-sm text-gray-500 break-all">
                                        {{ receptionist.user.email ?? 'No email entry' }}
                                    </td>
                                    <td class="p-4 text-sm text-gray-500 break-all">
                                        {{ receptionist.user.phone ?? 'No phone entry'}}
                                    </td>
                        ...
                        <Pagination :data="receptionists" v-if="receptionists && receptionists.data.length && receptionists.total > 20"/>
                    </div>
                </div>
            </div>
        </div>
        ...
        <NothingFound v-else>
            <BaseButton @click="showCreateDrawer = true" class="text-white bg-blue-600 hover:bg-blue-700 px-4 py-2">
                <font-awesome-icon icon="fa-solid fa-plus" class="h-4 w-4 mr-2"/>
                {{ __('Add Receptionists') }}
            </BaseButton>
       </NothingFound>

        <CreateReceptionist v-show="showCreateDrawer" :show="showCreateDrawer" @close-drawer="showCreateDrawer = false"/>
        <EditReceptionist v-show="showEditDrawer" :show="showEditDrawer" @close-drawer="showEditDrawer = false" :receptionist="editPharmacist"/>
    </AppLayout>
</template>

<script>
    import CreateReceptionist from "./Create.vue";
    import EditReceptionist from "./Edit.vue";
    ...
    export default {
        ...
        methods: {
            deleteData(id) {
                this.$swal({
                    ...
                    if (result.isConfirmed) {
                        this.$inertia.delete(route("admin.receptionist.destroy", id));
                    }
                });
            },
            editData(data){
            ...
            toggleFilter() {
                ...
                localStorage.setItem("adminReceptionist", this.showFilter);
            },
            changeViewType(type){
                this.viewType = type
                localStorage.setItem("adminReceptionistView", this.viewType);
            },
        },
        created() {
            this.showFilter = localStorage.getItem("adminReceptionist") == "true" ? true: false;
            this.viewType = localStorage.getItem("adminReceptionistView") == "card" ? 'card': 'table';
        },
    };
</script>
