<template>
    <AppLayout title="Department">
        ...
        <!-- Header Part  -->
        <Breadcrumb>
            <BreadcrumbLink title="Department"/>
        </Breadcrumb>
       ...
       <template v-else-if="!loading && departments && departments.data.length && viewType == 'card'">
           <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
               <span v-for="department in departments.data" :key="department.id" class="block p-6 bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100   ">
                   <div class="flex flex-wrap justify-between items-start">
                       <div class="relative mb-5">
                           <span>
                               <img class="w-16 h-16 rounded object-cover" alt="image" :src="department.image">
                           </span>
                       </div>
                       <Menu as="div" class="relative inline-block text-left">
                           <div>
                   ...
                   <h2 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 ">{{ department.name }}</h2>
        ...
        <!-- Table View  -->
        <BaseTable v-else-if="!loading && departments && departments.data.length && viewType == 'table'" :items="departments">
            ...
            <template v-slot:body>
                <tr v-for="department in departments.data" :key="department.id" class="divide-x divide-gray-200">
                    <td class="py-4 pl-4 pr-4 text-sm font-medium text-gray-900 sm:pl-6">
                        <div class="flex items-center">
                            <img class="h-16 w-16 rounded-md object-cover" :src="department.image" alt="">
                            <div class="ml-4">
