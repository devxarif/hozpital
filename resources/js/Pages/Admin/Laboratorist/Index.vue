    ...
    <AppLayout>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="#"
                            class="text-gray-700 hover:text-gray-900 ml-1 md:ml-2 text-sm font-medium">Laboratorist</a>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="mb-4 flex justify-between">
            ...
            <div class="flex items-center space-x-2 sm:space-x-3 ml-auto">
                ...
                <BaseButton v-if="filter.keyword && filter.keyword.length" as="link" :href="route('admin.laboratorist.index')" class="text-white bg-red-600 hover:bg-red-700 px-3 py-2">
        ...
        <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-100" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
            <div v-if="showFilter" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-5 mb-4 bg-white rounded-lg shadow-xs dark:bg-gray-800 items-center p-4">
                <div>
                    ...
                    <div class="mt-1">
                        <input v-model="filterForm.keyword" type="text" id="keyword" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm py-2.5" placeholder="Laboratorist name, email">
                    </div>
                </div>
                <div>
        ...
        <!-- Body Part  -->
       <template v-else-if="!loading && laboratorists && laboratorists.data.length && viewType == 'card'">
            <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
                <span v-for="laboratorist in laboratorists.data" :key="laboratorist.id" class="block p-6 bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                    <div class="flex flex-wrap justify-between items-start">
                        <div class="relative mb-5">
                            <span>
                                <img class="w-16 h-16 rounded object-cover" alt="image" :src="laboratorist.avatar">
                            </span>
                        </div>
                        <Menu as="div" class="relative inline-block text-left">
                            <div>
                    ...
                    <h2 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ laboratorist.user?.name ?? '-' }}</h2>
...
<script>
    import CreateLaboratorist from "./Create.vue";
    import EditLaboratorist from "./Edit.vue";
    ...
    export default {
        ...
        methods: {
            deleteData(id) {
                this.$swal({
                    ...
                    if (result.isConfirmed) {
                        this.$inertia.delete(route("admin.laboratorist.destroy", id));
                    }
                });
            },
            editData(data){
                this.showEditDrawer = true
                this.editLaboratorist = data
            },
            filterData(){
                this.loading = true
                this.filterForm.get(route('admin.laboratorist.index'), {
                    onSuccess: () => {
                        this.loading = false
                    },
                    onError: () => {
            ...
            toggleFilter() {
                ...
                localStorage.setItem("adminLaboratorist", this.showFilter);
            },
            changeViewType(type){
                this.viewType = type
                localStorage.setItem("adminLaboratoristView", this.viewType);
            },
        },
        created() {
            this.showFilter = localStorage.getItem("adminLaboratorist") == "true" ? true: false;
            this.viewType = localStorage.getItem("adminLaboratoristView") == "card" ? 'card': 'table';
        },
    };
</script>
