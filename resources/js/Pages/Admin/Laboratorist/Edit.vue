                        ...
                        <div class="flex-1 overflow-y-auto py-6 px-4 sm:px-6">
                            ...
                            <form class="mb-4" @submit.prevent="saveData">
                                <div class="grid grid-cols-2 gap-2">
                                    <div class="mb-4">
                                        <Label :name="__('Name')" id="laboratorist_name" :hasError="form.errors.name"/>
                                        <BaseInput v-model="form.name" placeholder="Name" id="laboratorist_name" :hasError="form.errors.name"/>
                                    </div>
                                    <div class="mb-4">
                                        <Label :name="__('Email')" id="laboratorist_email" :hasError="form.errors.email"/>
                                        <BaseInput v-model="form.email" placeholder="Email Address" id="laboratorist_email" :hasError="form.errors.email"/>
                                    </div>
                                </div>
                                <div class="mb-4">
                                    <label for="laboratorist_password">
                                    ...
                                    <div class="relative">
                                        <BaseInput v-model="form.password" placeholder="Password" id="laboratorist_password" :hasError="form.errors.password" :type="passwordFieldType"/>
...
export default {
    ...
    watch: {
        laboratorist: {
            handler() {
                this.form.name = this.laboratorist.user?.name ?? ''
                this.form.email = this.laboratorist.user?.email ?? ''
                this.previewImage = this.laboratorist.avatar
            },
            deep: true,
        },
    },
    data() {
        return {
            form: this.$inertia.form({
                name: this.laboratorist.user?.name ?? '',
                email: this.laboratorist.user?.email ?? '',
                password: "",
                avatar: '',
                _method: 'PUT'
            }),

            previewImage: this.laboratorist.avatar,
    ...
    methods: {
        ...
        saveData() {
            this.form.post(route("admin.laboratorist.update", this.laboratorist.id), {
                onSuccess: () => {
                    this.form.reset(),
                    this.removeImage()
