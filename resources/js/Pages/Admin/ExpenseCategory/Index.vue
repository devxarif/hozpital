<template>
    ...
    <AppLayout>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="#"
                            class="text-gray-700 hover:text-gray-900 ml-1 md:ml-2 text-sm font-medium">Expense Category</a>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-100" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
            <div v-if="showFilter" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-5 mb-4 bg-white rounded-lg shadow-xs dark:bg-gray-800 items-center p-4">
                <div>
                    ...
                    <div class="mt-1">
                        <input v-model="filterForm.keyword" type="text" id="keyword" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm py-2.5" placeholder="Expense category name">
                    </div>
                </div>
                <div>
       ...
       <template v-else-if="!loading && expense_categories && expense_categories.data.length">
           <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
               <span v-for="expense_category in expense_categories.data" :key="expense_category.id" class="block p-6 bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                   <div class="flex flex-wrap justify-between items-start">
                        <h2 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ expense_category.name }}</h2>
                       <Menu as="div" class="relative inline-block text-left">
                           <div>
                           ...
                           <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                               <MenuItems class="absolute right-0 z-10 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                                   <div class="py-1 text-sm">
                                   <MenuItem v-slot="{ active }">
                                       <a href="javascript:void(0)" @click.prevent="editData(expense_category)" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
                                   ...
                                   <MenuItem v-slot="{ active }">
                                       <a href="javascript:void(0)" @click.prevent="deleteData(expense_category.id)" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
       ...
       <NothingFound v-else>
        ...
        <CreateExpense :show="showCreateDrawer" @close-drawer="showCreateDrawer = false"/>
        <EditExpense :show="showEditDrawer" @close-drawer="showEditDrawer = false" :expense_category="editExpenseCategory"/>
...
export default {
    ...
    methods: {
        deleteData(id) {
            this.$swal({
                ...
                if (result.isConfirmed) {
                    this.$inertia.delete(route("admin.expenseCategory.destroy", id));
                }
            });
        },
        ...
        filterData(){
            this.loading = true
            this.filterForm.get(route('admin.expenseCategory.index'), {
                onSuccess: () => {
                    this.loading = false
                },
                onError: () => {
        ...
        toggleFilter() {
            ...
            localStorage.setItem("adminExpenseCategory", this.showFilter);
        },
    },
    created() {
        this.showFilter = localStorage.getItem("adminExpenseCategory") == "true" ? true: false;
    },
};
</script>
