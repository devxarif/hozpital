    ...
    <AppLayout>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="#"
                            class="text-gray-700 hover:text-gray-900 ml-1 md:ml-2 text-sm font-medium">Income Category</a>
                    </div>
                </li>
            </ol>
        </nav>
        <div class="mb-4 flex justify-between">
            ...
            <div class="flex items-center space-x-2 sm:space-x-3 ml-auto">
                <BaseButton v-if="filter.keyword && filter.keyword.length" as="link" :href="route('admin.incomeCategory.index')" class="text-white bg-red-600 hover:bg-red-700 px-3 py-2">
                ...
                <BaseButton as="link" :href="route('admin.income.index')" class="text-white bg-green-500 hover:bg-green-700 px-4 py-2.5">
                    <font-awesome-icon icon="fa-solid fa-plus" class="h-4 w-4 mr-2"/>
                    {{ __('Income List') }}
                </BaseButton>
        ...
        <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-100" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
            <div v-if="showFilter" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-5 mb-4 bg-white rounded-lg shadow-xs dark:bg-gray-800 items-center p-4">
                <div>
                    ...
                    <div class="mt-1">
                        <input v-model="filterForm.keyword" type="text" id="keyword" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm py-2.5" placeholder="Income category name">
                    </div>
                </div>
                <div>
       ...
       <template v-else-if="!loading && income_categories && income_categories.data.length">
           <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
               <span v-for="income_category in income_categories.data" :key="income_category.id" class="block p-6 bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                   <div class="flex flex-wrap justify-between items-start">
                        <h2 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ income_category.name }}</h2>
                       <Menu as="div" class="relative inline-block text-left">
                           <div>
                           ...
                           <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                               <MenuItems class="absolute right-0 z-10 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                                   <div class="py-1 text-sm">
                                   <MenuItem v-slot="{ active }">
                                       <a href="javascript:void(0)" @click.prevent="editData(income_category)" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
                                   ...
                                   <MenuItem v-slot="{ active }">
                                       <a href="javascript:void(0)" @click.prevent="deleteData(income_category.id)" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
           ...
           <Pagination :data="income_categories" v-if="income_categories && income_categories.data.length && income_categories.total > 20" class="mt-5"/>
       </template>
       ...
       <NothingFound v-else>
        ...
        <EditDepartment :show="showEditDrawer" @close-drawer="showEditDrawer = false" :income_category="editIncomeCategory"/>
...
export default {
    ...
    methods: {
        deleteData(id) {
            this.$swal({
                ...
                if (result.isConfirmed) {
                    this.$inertia.delete(route("admin.incomeCategory.destroy", id));
                }
            });
        },
        editData(incomeCategory){
            this.showEditDrawer = true
            this.editIncomeCategory = incomeCategory
        },
        filterData(){
            this.loading = true
            this.filterForm.get(route('admin.incomeCategory.index'), {
                onSuccess: () => {
                    this.loading = false
                },
                onError: () => {
        ...
        toggleFilter() {
            ...
            localStorage.setItem("adminIncomeCategory", this.showFilter);
        },
    },
    created() {
        this.showFilter = localStorage.getItem("adminIncomeCategory") == "true" ? true: false;
    },
};
</script>
