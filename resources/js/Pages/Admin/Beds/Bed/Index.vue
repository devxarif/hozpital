    ...
    <AppLayout title="Bed">
        ...
        <div class="mb-4 flex justify-between">
            ...
            <div class="flex items-center space-x-2 sm:space-x-3 ml-auto">
                ...
                <!-- Clear Filter -->
                <ClearFilter v-if="filter.keyword || filter.bed_type || filter.bed_floor || filter.status" :href="route('admin.bed.index')"/>
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <Multiselect id="admin_bed_status" :close-on-select="true" :can-clear="true"
                            ...
                            placeholder="Status" :options="[{ value: 'alloted', label: 'Alloted' },{ value: 'available', label: 'Available' },]"  />
                    </div>
                </div>
                <div>
            ...
            <div class="hidden sm:block">
                <div class="border-b border-gray-200">
                    <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                        ...
                        <button v-for="bed_type in bed_types" :key="bed_type.id" type="button" @click="changeTab(bed_type.slug)"  :class="['whitespace-nowrap flex py-4 px-1 border-b-2 font-medium text-sm focus:outline-none', currentTab == bed_type.slug ? 'border-blue-500 text-blue-600':'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-200' ]">
                            {{ bed_type.name }}
                            <span class="hidden ml-3 py-0.5 px-2.5 rounded-full text-xs font-medium md:inline-block" :class="currentTab == bed_type.slug ? 'bg-blue-100 text-blue-600':'bg-gray-100 text-gray-900'">
                                {{ bed_type.beds_count }}
                            </span>
                        </button>
                    </nav>
        ...
        <!-- Card View  -->
        <template v-else-if="!loading && beds && beds.data.length && viewType == 'card'">
            <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
                <span v-for="bed in beds.data" :key="bed.id" class="block p-6 bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100   ">
                    <div class="flex flex-wrap justify-between items-start">
                        <h2 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 ">{{ bed.bed_type?.name ?? '' }}</h2>
        ...
        <!-- Table View  -->
        <BaseTable v-else-if="!loading && beds && beds.data.length && viewType == 'table'" :items="beds">
            <template v-slot:head>
                <tr class="divide-x divide-gray-200">
                    <th class="py-3.5 pl-4 pr-4 text-left text-sm font-semibold text-gray-900 sm:pl-6">Bed Type</th>
                    <th class="px-4 py-3.5 text-left text-sm font-semibold text-gray-900 break-words">Number</th>
                    <th class="px-4 py-3.5 text-left text-sm font-semibold text-gray-900 break-words">Floor</th>
                    <th class="px-4 py-3.5 text-left text-sm font-semibold text-gray-900 break-words">Charge</th>
            ...
            <template v-slot:body>
                <tr v-for="bed in beds.data" :key="bed.id" class="divide-x divide-gray-200">
...
<script>
    ...
    export default {
        ...
        methods: {
            changeViewType(type){
                this.viewType = type
                localStorage.setItem("adminBed", this.viewType);
            },
            deleteData(id) {
                this.$swal({
                    title: "Are you sure?",
        ...
        created() {
            ...
            this.viewType = localStorage.getItem("adminBed") == "card" ? 'card': 'table';
        },
    };
</script>
