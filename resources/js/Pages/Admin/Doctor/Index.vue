    ...
    <AppLayout>
        ...
        <div class="mb-4 flex justify-between">
            ...
            <div class="flex items-center space-x-2 sm:space-x-3 ml-auto">
                ...
                <BaseButton v-if="filter.keyword && filter.keyword.length" as="link" :href="route('admin.doctor.index')" class="text-white bg-red-600 hover:bg-red-700 px-3 py-2">
                        ...
                        <MenuItems class="absolute right-0 z-10 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                            <div class="py-1 text-sm">
                                <MenuItem v-slot="{ active }">
                                    <a :href="route('admin.doctor.export', 'pdf')" target="_blank" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
                                        <font-awesome-icon icon="fa-solid fa-file-pdf" class="h-6 w-6 mr-2 text-red-500 group-hover:text-red-500"/>
                                        As PDF
                                    </a>
                                </MenuItem>
                                <MenuItem v-slot="{ active }">
                                    <a :href="route('admin.doctor.export', 'csv')" target="_blank" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
                                        <font-awesome-icon icon="fa-solid fa-file-csv" class="h-6 w-6 mr-2 text-blue-500 group-hover:text-blue-500"/>
                                        As CSV
                                    </a>
                                </MenuItem>
                                <MenuItem v-slot="{ active }">
                                    <a :href="route('admin.doctor.export', 'xlsx')" target="_blank" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2']">
       ...
       <template v-else-if="!loading && doctors && doctors.data.length  && viewType == 'card'">
        ...
        <!-- Table View  -->
        <div class="flex flex-col mb-5 shadow-lg" v-else-if="!loading && doctors && doctors.data.length && viewType == 'table'">
            <div class="-my-2 -mx-4 overflow-x-auto sm:-mx-6 lg:-mx-8">
                <div class="inline-block py-2 align-middle md:px-6 lg:px-8">
                    <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                        ...
                        <Pagination :data="doctors" v-if="doctors && doctors.data.length && doctors.total > 20"/>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            changeViewType(type){
                this.viewType = type
                localStorage.setItem("adminDoctorView", this.viewType);
            },
        },
        created() {
            this.showFilter = localStorage.getItem("adminDoctor") == "true" ? true: false;
            this.viewType = localStorage.getItem("adminDoctorView") == "card" ? 'card': 'table';
        },
    };
</script>
