@if ($paginator->hasPages())
    <ul class="pagination flex flex-wrap items-center justify-center">
        @if ($paginator->onFirstPage())
            <li class="mx-2">
                <button disabled type="button" class="cursor-not-allowed flex flex-wrap rounded-md items-center justify-center  w-11 h-11 bg-white shadow text-orange leading-none transition-all hover:bg-blue-500 hover:text-white" href="#">
            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    @if ($page == $paginator->currentPage())
                        <li class="mx-2">
                            <a class="flex flex-wrap rounded-md items-center justify-center w-11 h-11 bg-blue-500 shadow text-white leading-none transition-all" href="#">{{ $page }}</a>
                        </li>
                    @else
                        <li class="mx-2">
