...
class TeamController extends Controller
{
    ...
    {
        $teams = Team::select('id', 'name', 'organization_id')
            ->with('employees.user')
            ->where('organization_id', currentOrganization()->id)
            ->latest()
            ->paginate(10);

        return inertia('Organization/Teams', [
            'teams' => $teams,
        ]);
    }
    ...
    public function teamEmployees(Request $request)
    {
        if ($request->team && $request->team != 'all') {
            $team = Team::whereSlug($request->team)->where('organization_id', currentOrganization()->id)->firstOrFail();
