...
class LeaveRequestController extends Controller
{
    public function index()
    {
        ...
        $leave_types = LeaveType::where('organization_id', $employee->organization_id)->get(['id', 'name']);
    ...
    public function store(Request $request)
    {
        ...
        try {
            $employee = currentEmployee();
            $final_days_count = sumFinalDays($employee->organization_id, $request->start, $request->end) ?? diffBetweenDays($request->start, $request->end);
            ...
            // $leave_request->organization->user->notify(new NewLeaveRequest($leave_request, $employee->organization_id));
    ...
    {
        $employee = currentEmployee();
        $leave_types = LeaveType::where('organization_id', currentUser()->employee->organization_id)->get();
        $leaveTypeBalances = LeaveBalance::with('leaveType')->where('employee_id', $employee->id)->get();
    ...
    public function update(Request $request, LeaveRequest $leave_request)
    {
        ...
        try {
            $final_days_count = sumFinalDays($leave_request->organization_id, $request->start, $request->end) ?? diffBetweenDays($request->start, $request->end);
